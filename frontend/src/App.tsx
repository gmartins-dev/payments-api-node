...
export function App() {
  ...
  async function handleConcurrentRequests() {
    ...
    try {
      ...
      setAttempts((current) => [
        toAttemptRecord('Concurrent request #2', form.idempotencyKey, requestPayload, second, createdAt),
        toAttemptRecord('Concurrent request #1', form.idempotencyKey, requestPayload, first, createdAt),
        ...current
      ])
    } catch (error) {
  ...
  async function runSingleAttempt(label: string, idempotencyKey: string, payload: { amount: number; customerId: string }) {
    const validationError = validateForm(form)
    if (validationError) {
      setErrorMessage(validationError)
      return
    ...
    try {
      ...
      setAttempts((current) => [toAttemptRecord(label, idempotencyKey, payload, response, new Date().toISOString()), ...current])
    } catch (error) {
      setErrorMessage(readErrorMessage(error))
    } finally {
      setIsSubmitting(false)
          ...
          <div className="flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl space-y-4">
              ...
              <p className="max-w-2xl text-base leading-7 text-slate-300 sm:text-lg">
                This frontend drives the backend through fresh requests, same-key retries, and concurrent submissions so
                the idempotency behavior is visible instead of assumed.
              </p>
            </div>

            <div className="grid gap-3 sm:grid-cols-3">
              <Metric label="Backend target" value={apiUrl.replace(/^https?:\/\//, '')} />
              <Metric label="Latest status" value={latestAttempt ? String(latestAttempt.status) : 'Idle'} />
        ...
        <section className="grid gap-6 xl:grid-cols-[1.25fr_0.95fr]">
          <Card className="reveal">
            <CardHeader>
              ...
              <CardDescription>Use the same idempotency key to replay the stored result or trigger two parallel requests.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="flex flex-col gap-3 md:flex-row">
                ...
                <Button className="md:flex-1" disabled={isSubmitting} onClick={handleRetrySameRequest} variant="outline">
                  Retry same request
                </Button>
                <Button className="md:flex-1" disabled={isSubmitting} onClick={handleConcurrentRequests} variant="secondary">
              ...
              <div className="grid gap-4 lg:grid-cols-2">
                ...
                />
                <JsonPanel
                  caption={latestAttempt ? `HTTP ${latestAttempt.status}${latestAttempt.requestId ? ` • ${latestAttempt.requestId}` : ''}` : 'No response yet'}
          ...
          <Card className="reveal">
            <CardHeader>
              ...
              <CardDescription>Each entry records the request payload, HTTP status, and persisted response body returned by the API.</CardDescription>
            </CardHeader>
            <CardContent>
              {attempts.length === 0 ? (
                <div className="rounded-[24px] border border-dashed border-white/12 bg-slate-950/50 p-6 text-sm leading-6 text-slate-400">
                  No attempts yet. Start with a fresh key, then hit retry or concurrency to observe how the same stored result is reused.
                </div>
              ) : (
                    ...
                    >
                      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                          ...
                          <p className="mt-2 text-sm text-slate-400">{formatTimestamp(attempt.createdAt)}</p>
                        </div>
                      ...
                      <dl className="mt-4 grid gap-3 text-sm text-slate-300">
                        <div>
                          <dt className="text-xs uppercase tracking-[0.22em] text-slate-500">Idempotency key</dt>
                          <dd className="mt-1 break-all font-medium text-slate-100">{attempt.idempotencyKey}</dd>
                        </div>
                        <div>
                          <dt className="text-xs uppercase tracking-[0.22em] text-slate-500">Request payload</dt>
                        ...
                        <div>
                          <dt className="text-xs uppercase tracking-[0.22em] text-slate-500">Response body</dt>
                          <dd className="mt-2 overflow-x-auto rounded-2xl bg-slate-950/80 p-4 text-xs leading-6 text-slate-200">
...
async function sendPaymentRequest(idempotencyKey: string, payload: { amount: number; customerId: string }) {
