...
export class PaymentsService {
  ...
  private async processOwnedPayment(payment: PaymentRow, input: CreatePaymentInput) {
    try {
      ...
      const saved = await this.repository.markFailed(payment.id, 200, failedResponse, failedResponse.reason)
...
function replayPersistedResponse(payment: PaymentRow): { statusCode: number; body: PersistedPaymentResponse } {
  if (payment.status !== 'SUCCESS' && payment.status !== 'FAILED') {
