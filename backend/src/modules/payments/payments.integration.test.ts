...
class ControlledProcessor {
	readonly process = vi.fn(
		async (input: CreatePaymentInput, paymentId: string): Promise<PaymentSuccessResponse> => {
...
async function countPayments(pool: {
	query: (sql: string) => Promise<{ rows: Array<{ count: number }> }>
}) {
